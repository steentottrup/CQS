...
namespace CreativeMinds.CQS.Dispatchers {
	...
	public abstract class QueryDispatcherBase : IQueryDispatcher {
		...
		protected abstract IGenericValidationQueryHandlerDecorator<TQuery, TResult> GetValidationHandler<TQuery, TResult>(IQueryHandler<TQuery, TResult> wrappedHandler) where TQuery : IQuery<TResult>;
		protected abstract IGenericPermissionCheckQueryHandlerDecorator<TQuery, TResult> GetPermissionCheckHandler<TQuery, TResult>(IQueryHandler<TQuery, TResult> wrappedHandler) where TQuery : IQuery<TResult>;
		protected abstract IQueryHandler<TQuery, TResult> GetQueryHandler<TQuery, TResult>() where TQuery : IQuery<TResult>;

		protected QueryDispatcherBase(ILogger<QueryDispatcherBase> logger) {
			this.logger = logger;
		}
		...
		protected virtual IQueryHandler<TQuery, TResult> Resolve<TQuery, TResult>() where TQuery : IQuery<TResult> {
			...
			if (attrs.Any(a => a.GetType() == typeof(CreativeMinds.CQS.Decorators.ValidateAttribute))) {
				...
				IQueryHandler<TQuery, TResult> validationHandler = this.GetValidationHandler<TQuery, TResult>(handler);
