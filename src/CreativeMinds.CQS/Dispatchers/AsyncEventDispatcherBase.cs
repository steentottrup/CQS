...
namespace CreativeMinds.CQS.Dispatchers {
	...
	public abstract class AsyncEventDispatcherBase {
		...
		public void PublishAsync<TEvent>(TEvent eventMessage, CancellationToken cancellationToken) where TEvent : IEvent {
			...
			foreach (IAsyncEventHandler<TEvent> handler in handlers) {
				try {
					handler.HandleAsync(eventMessage, cancellationToken);
				}
				catch (Exception ex) {
